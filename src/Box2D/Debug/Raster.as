...
package Box2D.Debug
{
	...
	public class Raster extends BitmapData
	{
		...
		*/
		...
		{
			_buffer.length = 0;
			lineTri (_buffer,x0,y0,x1,y1,color);
			lineTri (_buffer,x1,y1,x2,y2,color);
			lineTri (_buffer,x2,y2,x0,y0,color);
		}
		...
		*/
		private function lineTri ( o:Array, x0:int, y0:int, x1:int, y1:int, c:Number ):void
		{
			...
			while (x++<=xend)
			{
				if (steep)
				{
					checkLine(o,y,x,c,_rect);
					if (fx != x1 && fx != xend)
						checkLine(o,fy,fx+1,c,_rect);
				}
				...
				if ((error<<1) >= deltax)
				{
					if (!steep)
					{
						checkLine(o,x-px+1,y,c,_rect);
						if (fx!=xend) 
							checkLine(o,fx+1,fy,c,_rect);
					}
					px = 0;
					y += ystep;
					fy -= ystep;
